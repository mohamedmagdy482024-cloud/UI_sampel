...
class ModernNumberInput(tk.Entry):
    ...
    def _apply_bg(self):
        if self._focused:
            self.config(bg=self.FOCUS_BG, highlightthickness=1, highlightbackground=self.ACCENT_COLOR)
        elif self._hovering:
            self.config(bg=self.HOVER_BG, highlightthickness=0)
        else:
            self.config(bg=self.BASE_BG, highlightthickness=0)
...
root = tk.Tk()
root.title("Modern Dark Dynamic Text & Number Inputs")
...
text_label = tk.Label(
    container,
    text="Text Input (left-aligned, placeholder support)",
    font=("Arial", 10, "bold"),
    bg="#1e222b",
    fg="#ffffff",
)
...
number_label = tk.Label(
    container,
    text="Number Input (center-aligned, numeric validation)",
    font=("Arial", 10, "bold"),
    bg="#1e222b",
    fg="#ffffff",
)
